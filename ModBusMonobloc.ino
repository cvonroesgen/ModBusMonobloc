...
// EXT_COIL_TEMP	03	External coil temperature	Unicode/double byte
// COOL_COIL_TEMP	03	Cooling coil temperature	Unicode/double byte
...
int led = 13;
int EN = 2;
unsigned long outDoorResetTimer = millis() - (outDoorResetIntervalMinutes * 60000L);
unsigned long ledDisplayTimer = millis();
const unsigned long dewPointUpdateMilliseconds = 1800L * 1000L;
unsigned long dewPointUpdateInterval = 5L * 1000L; //every 5 seconds initially
...
unsigned short menuCodes[NUM_MENU_ITEMS] = { ON_OFF, HOT_WATER_SET_POINT, WATER_TANK_TEMP, Outlet_water_temperature, Inlet_water_temperature, AMBIENT_TEMP, EXT_COIL_TEMP, COOL_COIL_TEMP, COIL_TEMP_FOR_DEFROST_MODE, FAN_SPEED, AC_VOLTS, AC_AMPS, NWS_DEW_POINT, NO_HEAT_TEMP_SET, C_UP_PER_C_DOWN, COP_CRC };
  ...
  if (buttons) {
    ...
    if (millis() - lastDebounceTime > debounceDelay) {
      ...
      if (menuCodes[menuIndex] >= ON_OFF) {
        requestDataFromMonoBus(menuCodes[menuIndex], &displayMonoBusGetResponse);
...
void read_http_response() {
  ...
  if (apiJSON.hasOwnProperty("properties")) {
    if ((String)apiJSON["properties"]["dewpoint"]["qualityControl"] == "V") {
      ...
      dewPointUpdateInterval = dewPointUpdateMilliseconds;
    }
  }
}
